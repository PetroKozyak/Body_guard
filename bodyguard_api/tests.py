...
from bodyguard_api.models import Job, GuardFirm, FirmFeedback, Order
    ...
    createJob = lambda: (
        ...
        (BaseTestCaseAuthUser.USER_ID_1,
         {"type_job": Job.REGULAR_JOB,
          ...
          "comment": "testcomment"},
         400,),
        (BaseTestCaseAuthUser.USER_ID_4_NOT_EXIST,
         {"type_job": Job.REGULAR_JOB,
          "title": "TestTitle",
          "number_guard": 2,
          "variant": [JobCreateAPIViewTestCase.OPTION_CAR_VARIANT_YES_ID,
...
class FirmCreateAPIViewTestCase(BaseTestCaseAuthUser):
    ...
    create_firm = lambda: ((BaseTestCaseAuthUser.USER_ID_1,
                            {"name": "TestFirm",
                             "comment": "testcomment"},
                            403,),
...
class FirmDetailAPIViewTestCase(BaseTestCaseAuthUser):
    create_firm = lambda: ((BaseTestCaseAuthUser.USER_ID_3,
                            {"owner_id": BaseTestCaseAuthUser.USER_ID_3,
                             "name": "TestFirm",
                             "comment": "testcomment"},
                            200,),
                           )
    ...
    @data_provider(create_firm)
    def test_update_firm(self, user_id, data, response_code):
        self.before_test(user_id)
        firm = GuardFirm.objects.create(**data)
        url = reverse("guard-detail", kwargs={"pk": firm.id})
    ...
    def test_delete_firm(self, user_id, data, response_code):
        self.before_test(user_id)
        firm = GuardFirm.objects.create(**data)
        response = self.client.delete(reverse("guard-detail", kwargs={"pk": firm.id}))
        self.assertEqual(response_code, response.status_code)
...
class FeedCreateAPIViewTestCase(BaseTestCaseAuthUser):
    ...
    create_feedback = lambda: ((BaseTestCaseAuthUser.USER_ID_3,
                                BaseTestCaseAuthUser.USER_ID_1,
                                {"owner_id": BaseTestCaseAuthUser.USER_ID_3,
                                 "name": "TestFirm",
                                 "comment": "testcomment"},
    ...
    def test_feedback_create(self, owner_id, user_id, data_firm, response_code):
        self.before_test(owner_id)
        firm = GuardFirm.objects.create(**data_firm)
        data = {
            "feedback": "not bad",
            "firm": firm.id,
        }
...
class FeedDetailAPIViewTestCase(BaseTestCaseAuthUser):
    ...
    @data_provider(create_feedback)
    def test_delete_feedback(self, firm_owner_id, feed_owner_id, firm_data, response_code):
        self.before_test(firm_owner_id)
        firm = GuardFirm.objects.create(**firm_data)
        owner_feed = self.before_test(feed_owner_id)
        data = {"customer": owner_feed,
                "feedback": "not bad",
                "firm": firm,
                }
        feed_back = FirmFeedback.objects.create(**data)

        response = self.client.delete(reverse("feed_back-detail", kwargs={"pk": feed_back.id}))
        self.assertEqual(response_code, response.status_code)
    ...
    @data_provider(create_feedback)
    def test_update_feedback(self, firm_owner_id, feed_owner_id, firm_data, response_code):
        self.before_test(firm_owner_id)
        firm = GuardFirm.objects.create(**firm_data)
        owner_feed = self.before_test(feed_owner_id)
        data = {"customer": owner_feed,
        ...
        feed_back = FirmFeedback.objects.create(**data)
        url = reverse("feed_back-detail", kwargs={"pk": feed_back.id})
        response = self.client.put(url, {"feedback": "test_update_feedback", "firm": firm.id})
        self.assertEqual(response_code, response.status_code)
...
class OrderCreateAPIViewTestCase(BaseTestCaseAuthUser):
    ...
    def test_create_order(self, firm_owner, job_owner, data_firm, data_job, response_code):
        self.before_test(job_owner)
        job = Job.objects.create(**data_job)
...
class OrderDetailAPIViewTestCase(BaseTestCaseAuthUser):
    ...
    @data_provider(create_order)
    def test_update_order(self, firm_owner, job_owner, data_firm, data_job, response_code):
        self.before_test(job_owner)
        job = Job.objects.create(**data_job)
        self.before_test(firm_owner)
        firm = GuardFirm.objects.create(**data_firm)
        ...
        response = self.client.put(url, {"job": job.id, "firm": firm.id, "price": 88888})
        if self.assertEqual(response_code, response.status_code):
            self.before_test(job_owner)
    ...
    @data_provider(create_order)
    def test_delete_order(self, firm_owner, job_owner, data_firm, data_job, response_code_not_approved,
        ...
        self.before_test(firm_owner)
        response = self.client.delete(reverse("orders-detail", kwargs={"pk": order.id}))
        if self.assertEqual(response_code_approved, response.status_code):
            self.before_test(job_owner)
            url = reverse("orders-detail", kwargs={"pk": order.id})
            self.client.put(url, {"approved": False})
            self.before_test(firm_owner)
            response = self.client.delete(reverse("orders-detail", kwargs={"pk": order.id}))
            self.assertEqual(response_code_not_approved, response.status_code)
...
class PayAPIViewTestCase(BaseTestCaseAuthUser):
    ...
    def test_create_stripe(self, job_owner, order, token, errors):
        ...
        self.client.put(url, {"approved": True})
        url = reverse("orders-pay", kwargs={"pk": order})
